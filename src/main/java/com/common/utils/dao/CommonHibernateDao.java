...
public class CommonHibernateDao extends HibernateDaoSupport {
    ...
    public Integer findCountByCriteria(final DetachedCriteria detachedCriteria) {
        @SuppressWarnings("unchecked")
        Integer count = (Integer) getHibernateTemplate().executeWithNativeSession(new org.springframework.orm.hibernate4.HibernateCallback() {
            @Override
...
//    },sql,null);
    ...
    public List<List<Object>> findListListBySQL(String sql, Object... values) {
        SQLQuery sqlQuery = (SQLQuery) createSQLQuery(sql, values);
        sqlQuery.setResultTransformer(Transformers.TO_LIST);
        return (List<List<Object>>) sqlQuery.list();
    }
